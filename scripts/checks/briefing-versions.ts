...
try{
 await c.query('BEGIN');
 for(const table of ['items','briefing_versions','translations'])await c.query(`CREATE TEMP TABLE ${table} (LIKE public.${table} INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES) ON COMMIT DROP`);
