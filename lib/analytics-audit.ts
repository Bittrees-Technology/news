...
export async function analyticsAudit() {
 const [coverage, recent, feedback, deliveries, availability, shadow, engagement] = await Promise.all([
 ...
 return {at:new Date().toISOString(),catalogSize:sources.length,shadow:shadow.rows[0],engagement,availability:availability.rows.filter(r=>sources.some(s=>s.id===r.source_id)),coverage:coverage.rows[0],recent:{hours:24,total,activeSources:recent.rows.length,topSourceShare:total?recent.rows[0].items/total:null,sources:recent.rows.slice(0,10)},feedback:{days:30,...feedback.rows[0]},deliveries:{days:7,statuses:deliveries.rows}};
}
