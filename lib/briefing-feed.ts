...
export async function briefingBatch(before:string|null=null,selected:string|null=null,snapshot=new Date().toISOString()){
 const rows=(await pool().query(`SELECT i.*,s.document,s.cid FROM items i JOIN story_documents s ON s.item_id=i.id WHERE s.document IS NOT NULL AND i.owner_id IS NULL AND i.published_at<=$3 AND i.fetched_at<=$3
