...
export async function claimTranslation() {
  ...
  ) UPDATE translations t SET status='working',claimed_at=now(),lease=gen_random_uuid(),attempts=attempts+1 FROM candidate c WHERE t.key=c.key RETURNING t.key,t.lease,t.payload,t.priority`);
  const row=r.rows[0];return row?{...row,job:publicJob('translation',row.key,row.payload,row.lease,row.priority)}:null;
}
