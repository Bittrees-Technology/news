...
export async function api(r: Request) {
  try {
    ...
    if(path==='editor/telemetry' && method==='POST'){
      ...
      const data=z.object({completed:z.number().int().min(0).max(100000),failed:z.number().int().min(0).max(100000),median_seconds:z.number().min(0).max(10000).nullable(),managed_model:z.string().max(100).nullable(),smtp_local:z.boolean().optional(),smtp_public_route:z.boolean().optional(),event_samples:z.number().int().min(0).max(1000).optional(),benchmark_samples:z.number().int().min(0).max(1000).optional(),unknown_samples:z.number().int().min(0).max(1000).optional(),sample_start:z.string().datetime().nullable().optional(),sample_end:z.string().datetime().nullable().optional()}).parse(await body(r));
