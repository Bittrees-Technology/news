...
export function globalRelevance(item: Item): number {
  ...
  if (geography.countries.length >= 2 && /\b(trade|econom\w*|markets?|diploma\w*|treaty|treaties|sanctions?|war|conflict|attacks?|security|migration|climate|energy|research|health|technology|regulation|policy)\b/i.test(text)) return 85;
