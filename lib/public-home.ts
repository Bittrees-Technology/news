...
export const publicHome=unstable_cache(async():Promise<Edition|null>=>{
 ...
 const [edition,ranked,batch]=await Promise.all([latestEdition(),recentPublicRanked(snapshot),publicFeedBatch(snapshot)]);
 if(!edition)return null;
 const leads=await feedDisplay(ranked.slice(0,3));
 const leadIds=leads.map(i=>i.id);
 return JSON.parse(JSON.stringify({...edition,data:{...edition.data,items:feedOrder([...leads,...batch.items],leadIds)},feedPage:{next:batch.next,snapshot,leadIds}}));
},['public-home-v2'],{revalidate:30});
