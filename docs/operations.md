...
The long-briefing worker allows up to 20 minutes for the CPU model, caps generation at 400 tokens, and uses a shorter evidence window. Structured World Bank/DeFiLlama observations use deterministic source-linked briefings without model calls. These are explicitly identified as structured-data briefings. The public IPFS proof was created through the same result/pin/receipt functions; narrative-model processing may take several minutes per story.
