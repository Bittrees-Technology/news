...
### 21 September 20:52 UTC — evening publication and restrained monitoring

The 19:57 edition published at 19:57:27.238 UTC, within the two-minute target. Diversity shadow now contains seven editions, not seven days; evaluation still cannot begin before 27 September 02:05 UTC. At 20:50, briefing queue count fell to 135 (108 current-day), while oldest known wait increased to 460 minutes. Five new translations, no unknown enqueue timestamps. Lower queue count alone does not establish aging/fairness recovery or a cadence-caused speedup. Bounded briefing logs since 20:00 show two truncated outputs, two capacity deferrals and one model HTTP502; no output-limit, prompt or validation change follows from this small interval.

All four News services active, supervisor healthy, candidate unloaded. SMTP report at 20:47 fresh with both host-origin probes passing; ten identified production failures and 15 historical unknown events in its bounded window. Latest collection: 48 successes, zero failures, 34 unchanged responses in 13 seconds. Conditional totals: 7,871 HTTP304s / 16,333 checks, 4,003,551,856 transferred bytes, without controlled savings claims. Seven HTTP403 sources remain restricted; Hacker News has an intermittent fetch failure again. Used stored telemetry and local logs only: no publisher probes, increased polling/concurrency or added sources. The user's source-restraint requirement remains in the plan and automation; publisher-wide request budgets and adaptive low-yield cadence are still follow-up work, not asserted controls.

Zero signed provider events; existing provider-admin dependency unchanged. Completed expanded-source assessment retained. No runtime/model/ranking changes or deployment this checkpoint; website `2f7e30c`, diagnostic worker `b29ab1d`, cadence `4f384ff`, supervisor `c258469`. Durable dependency/capacity work, isolated truncation quality/timing comparison, evidence/source-policy and remaining acceptance gates stay open.


### 21 September 21:53 UTC — publisher rate-limit restraint

Stored telemetry showed Rest of World returned HTTP429 at 21:40:11.653 UTC with its first retry due 22:10. Source-restraint increment `50b3321` raises the minimum HTTP429 cooldown to 60 minutes while retaining any longer existing exponential/base/Retry-After delay within the current parser bounds. Tests cover absent/short Retry-After, a two-hour request, mature exponential backoff and a daily source. 69 tests and production build passed. A guarded update extended the one currently rate-limited, unleased source's next check to 22:40:11.653; longer existing schedules were not shortened. No publisher probe, added source, concurrency increase or ranking change. Publisher-family budgets and adaptive low-yield cadence remain separate unfinished work.

21:50 queue: 96 eligible briefings (70 current-day), oldest known wait 505 minutes; four newly queued translations with known enqueue ages. Queue volume is falling while oldest age still increases, so fairness/backlog acceptance is incomplete. Local logs since 21:00 show one truncated output, one capacity deferral and one model HTTP502. All four News services active, supervisor healthy, candidate unloaded. SMTP report at 21:47 fresh, both host-origin probes passing; 12 identified production failures and 13 historical unknown events in the bounded relay. Latest collection: 48 successes, zero failures, 36 unchanged in 13 seconds. Cumulative conditional totals 8,050 HTTP304s / 16,702 checks and 4,095,717,153 bytes, without a controlled savings claim. Seven HTTP403 restrictions and intermittent Hacker News failure persist.

Seven diversity shadows (still below seven days), zero signed provider events; existing provider-admin dependency unchanged. Evening publication already verified within target; completed source assessment retained. Isolated truncation comparison, durable dependencies/capacity, quality/evidence/source-policy and other acceptance gates remain open. Worker diagnostics `b29ab1d`, cadence `4f384ff`, supervisor `c258469` unchanged.

Production alias and health verified for `50b3321`. No schema migration or worker restart needed. This implements a conservative rate-limit floor, not a complete publisher-wide request budget or completion of either plan.


### 21 September 22:54 UTC — cooldown verified from stored observations

Rest of World's rate-limit recovery confirms the longer scheduled pause: unavailable at 21:40:11.653, first subsequent recorded check healthy at 22:40:17.965, then healthy at 22:55:12.292. No intervening recorded observation and no manual publisher probe. This verifies the observed cooldown/recovery sequence, not a guarantee against future rate limits. Seven HTTP403 restrictions remain under normal backoff; Hacker News still has a fetch failure. Conditional totals: 8,237 HTTP304s / 17,101 checks, 4,192,614,132 bytes, without controlled savings claims. Latest collection: 31 successes, no failures, 23 unchanged responses in eight seconds.

22:55 queue: 69 eligible briefings (38 current-day), oldest known wait 545 minutes; six new translations and no unknown enqueue timestamps. Volume continues down but oldest wait still grows; fairness acceptance remains incomplete. Four services active, supervisor healthy, candidate unloaded. SMTP telemetry fresh at 22:52 with both host-origin probes passing; 13 identified production model failures and 11 historical unknown events in the bounded relay. Seven diversity shadows, zero signed provider events; provider-admin and seven-day dependencies unchanged. Completed source assessment retained.

User-requested Summary reader deployed separately as website `f3d2e37`, preserving explicit refresh and guest-local read history; validation details in `docs/summary-reader-2026-09-21.md` (72 tests/build passed, visual browser validation blocked by administrator-policy verification). Collection cooldown `50b3321` remains included. Worker diagnostics `b29ab1d`, cadence `4f384ff`, supervisor `c258469` unchanged. No new runtime/ranking/model changes this checkpoint. Isolated truncation quality/timing comparison, durable dependencies/capacity, publisher-family budgets, evidence/source-policy and remaining acceptance gates stay open.
