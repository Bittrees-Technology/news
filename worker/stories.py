...
def brief(j):
 ...
 with urllib.request.urlopen(req,timeout=1200) as r:out=json.load(r)['choices'][0]['message']['content']
...
while True:
 try:
  ...
  if not document:document=api('result',{'id':job['item_id'],'briefing':brief(job),'model':('Bittrees structured-data briefing' if job['kind']=='data' and job['source_id'] in ('world-bank-gdp','defillama-protocols') else 'Bittrees-hosted '+config['model'])})
