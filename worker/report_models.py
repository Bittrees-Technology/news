...
import json,pathlib,statistics,time,urllib.request,socket
...
while True:
    try:
        ...
        payload={'smtp_local':smtp_check('127.0.0.1'),'smtp_public_route':smtp_check('smtp.bittrees.org'),'completed':len(good),'failed':len(rows)-len(good),'median_seconds':round(statistics.median(times),2) if times else None,'managed_model':health.get('managed_model')}
